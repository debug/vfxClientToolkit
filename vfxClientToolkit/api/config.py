...
CONFIG_TEMPLATE_DIR = Path(os.path.join(os.path.split(vfxClientToolkit.__file__)[0], "configs"))
...
class ConfigBundle(object):
    ...
    def getTemplates(self):
        ...
        for config in CONFIG_TEMPLATE_DIR.glob("*.json"):
            fh = open(config, 'r')
            configData = json.loads(fh.read())
            templates[list(configData.keys())[0]] = configData[list(configData.keys())[0]]
