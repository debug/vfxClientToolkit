...
def copySupportFiles(path, file):
    ...
    if os.path.exists(path):
        combinedPath = os.path.join(CONFIG_DATA['filesystem']['settings']['project_tree_root'], SHOT_SEQ_STRUCTURE)
        destinationPath = combinedPath.format(PROJECT=CONFIG_DATA['shotgun']['settings']['project_name'], SEQ=file['seq'].upper(), SHOT=file['shot'].upper(), DEPT=file['dept'])
        destinationSupportPath = os.path.join(destinationPath, "{0}/SupportFiles".format(file['version_name']))
...
def copyEXRs(path, file):
    if os.path.exists(path):
        ...
        combinedPath = os.path.join(CONFIG_DATA['filesystem']['settings']['project_tree_root'], SHOT_SEQ_STRUCTURE)
        destinationPath = combinedPath.format(PROJECT=CONFIG_DATA['shotgun']['settings']['project_name'], SEQ=file['seq'].lower(), SHOT=file['shot'].upper(), DEPT=file['dept'])
        versionDir = os.path.join(destinationPath, "{0}/exrs".format(file['version_name']))
        ...
        exrDestination = os.path.join(versionDir, "{0}.####.exr".format(file['version_name']))

        return exrDestination

def copyMov(file, infoDict):
    combinedPath = os.path.join(CONFIG_DATA['filesystem']['settings']['project_tree_root'], SHOT_SEQ_STRUCTURE)
    destinationPath = combinedPath.format(PROJECT=CONFIG_DATA['shotgun']['settings']['project_name'], SEQ=infoDict['seq'], SHOT=infoDict['shot'].upper(), DEPT=infoDict['dept'])
    try:
        os.makedirs(destinationPath)
    except:
            pass

    versionDir = os.path.join(destinationPath, "{0}/mov".format(infoDict['version_name']))
