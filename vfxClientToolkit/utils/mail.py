...
def sendMail(data, subject, recipients, attach=[]):
    ...
    message = Message(From=CONFIG_DATA['email']['settings']['username'], To=recipients, attachments=attach)
    message.Subject = subject
    message.Html = data
    sender = Mailer(host=CONFIG_DATA['email']['settings']['smtp'], port=CONFIG_DATA['email']['settings']['port'], use_tls=True)
    sender.login(CONFIG_DATA['email']['settings']['username'], CONFIG_DATA['email']['settings']['password'])
    sender.send(message)
